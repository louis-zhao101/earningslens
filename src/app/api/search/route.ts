...
export async function GET(request: NextRequest) {
  ...
  // Fall back to FMP search (symbol first, then name)
  try {
    ...
    const merged = [...bySymbol, ...byName].filter((r) => {
      if (seen.has(r.symbol) || !r.exchange.includes("NASDAQ") && !r.exchange.includes("NYSE") && !r.exchange.includes("NYSE")) return false;
      seen.add(r.symbol);
      return true;
    });
    const results = merged.slice(0, 8).map((r) => ({
      ticker: r.symbol,
      name: r.name,
      sector: null,
      exchange: r.exchange,
